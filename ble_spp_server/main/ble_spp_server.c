        ...
        if (xQueueReceive(spp_uart_queue, (void * )&event, (portTickType)portMAX_DELAY)) {
            ...
            case UART_DATA:
                if ((event.size)&&(is_connected)) {
                    ...
                    ESP_LOGI(GATTS_TABLE_TAG, "[UART]: %c", temp[0]);
...
void spp_heartbeat_task(void * arg)
{
        ...
        if(xQueueReceive(cmd_heartbeat_queue, &cmd_id, portMAX_DELAY)) {
            while(1){
                ...
                if(is_connected && enable_heart_ntf){
                    ESP_LOGI(GATTS_TABLE_TAG, "%s %d - %s", __func__, __LINE__, heartbeat_s);
    ...
    ESP_LOGE(GATTS_TABLE_TAG, "Exit %s %d", __func__, __LINE__);
    vTaskDelete(NULL);
}
#endif
    	...
    	case ESP_GATTS_WRITE_EVT: {
            ...
            if(p_data->write.is_prep == false){
                ...
                if(res == SPP_IDX_SPP_COMMAND_VAL){
                    ...
                    ESP_LOGI(GATTS_TABLE_TAG, "%s %d - SPP_IDX_SPP_DATA_RECV_VAL", __func__, __LINE__);
