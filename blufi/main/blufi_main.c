...
static esp_err_t example_net_event_handler(void *ctx, system_event_t *event)
{
    ...
    BLUFI_INFO("example_net_event_handler [event: 0x%x]", event->event_id);

    switch (event->event_id) {
    case SYSTEM_EVENT_STA_START:
        BLUFI_INFO("Wifi Start");
        esp_wifi_connect();
        break;
...
static void example_event_callback(esp_blufi_cb_event_t event, esp_blufi_cb_param_t *param)
{
    ...
    BLUFI_INFO("example_event_callback [event: 0x%x]", event);
